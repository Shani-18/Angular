...
export class TodoAddFormComponent implements OnInit {
  ...
  onBtnAddToDoItemClick(): void {
    ...
    todoItem.taskName = (<HTMLInputElement>(
      this.txtTaskName.nativeElement
    )).value;
    todoItem.dueDate = new Date(
      Date.parse((<HTMLInputElement>this.txtDueDate.nativeElement).value)
    );
    todoItem.isCompleted = (<HTMLInputElement>(
      this.chkCompleted.nativeElement
    )).checked;
